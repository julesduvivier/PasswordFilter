...
void Log(wchar_t * format, ...) {
    ...
    wcsftime(timeBuffer, sizeof(timeBuffer), L"%d/%m/%y %I:%M%p", timeinfo);
...
BOOLEAN CheckTokenizedStringInWordlist(PUNICODE_STRING password, wchar_t *file)
{
    ...
    SecureZeroMemory(passwordContent, sizeof(passwordContent));
